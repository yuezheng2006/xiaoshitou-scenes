...
Roles: Lao Yang points/annotates/explains; Little Stone executes/carries/labels as optional helper — NOT mandatory handshake dual-IP scene every frame.
...
Lao Yang inside the diagram — pointing, annotating, walking the path — NOT a tiny corner sticker.
If Combo: Little Stone may move modules / stick labels as flat 2D helper inside the same vinyl scene.
Budget: 1 core structure + 3-5 modules + short Chinese labels. Readable at article width.
...
ONE decisive moment / metaphor gesture. Sparse text (0-3 short labels).
...
```text
One-theme two-plates (一鱼两吃) — same theme, same character sheet identity:
...
```text
Spirit Face Lock (theater — 神似 vinyl-toy, 憨厚, not photo chase):
...
NO buzz-cut hair, NO widow's peak, NO thick black/dark-brown glasses frames,
NO skinny 175cm average stocky toy, NO 1:1 torso-legs squat, NO K/J idol face, NO baby-face,
NO flat plane face / low cheekbones / Korean beauty filter,
