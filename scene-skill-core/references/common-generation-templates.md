...
The capsule must have a continuous visible thin black outline (same family as limb strokes): present, not missing/gray, not bold marker, not a rectangular sticker frame. Pose is free; lock identity and proportion only.
Negative: no 3D mascot, no emoji 3D, no blind-box figure, no inflatable toy, no game character render, no Blender/C4D look, no gradient on profile-primary-color body, no specular highlight on profile-primary-color body, no orange volume lighting, no shaded orange capsule. Even a mild orange gradient on the body fails the flat lock. No missing body outline, no extra-thick marker outline.
```
...
Physical: must attach author-persona-panorama.png. Handdrawn/knowledge/PPT: must attach author-persona-panorama-handdrawn.png + author-persona-panorama.png (same person). Multi-scene batch: also attach author-persona-face-lock.png. Full-body handdrawn: also attach author-persona-handdrawn-body.png for body proportions.
Lao Yang = Northern Chinese / Shandong male face (wide solid jaw, NOT Korean idol / V-line / pale K-beauty), large thin LIGHT-GRAY rectangular glasses (NOT thick black frames), short straight black hair 3-5cm, beige short-sleeve T-shirt + olive khaki shorts + off-white canvas sneakers, thicker lower lip, tan skin, mature Chinese male about 38 years old, restrained stylized 3D in physical panels / flat black-line in sketch panels. Front or 3/4 face visible in hand-drawn/knowledge-card/PPT — NO back view, NO speech bubble only without face. Expression: calm, focused, slightly serious; lips closed or micro-explain without teeth; NO exaggerated smile or marketing grin. Cross-scene: same exact person, minimal face/outfit variation. Do NOT invent a generic business man, cartoon lecturer, black long-sleeve shirt, grey sweatpants, or thick black glasses. If panel is too small for a locked portrait, use abstract dual-IP symbols only (labels, arrows, silhouettes) — NO generic human face.
...
Negative persona: thick black glasses, black sweater/long sleeve, grey sweatpants, Korean idol face, generic avatar, photo-real face, chibi/clay toy, wrong outfit, back view, toothy smile, oversized head / short legs, old-lady silhouette.
```
...
- For panels about dual IP: either (A) use abstract symbols only — arrows, role labels「老杨讲」「小石头干」, orange capsule icon for Little Stone, NO human face; OR (B) if drawing Lao Yang, MUST attach panorama assets per mode (+ face-lock for multi-scene; + handdrawn-body for full-body handdrawn) and follow Persona Identity Lock above exactly.
...
{IP_DESC} — paste from active profile character.md (after attaching Little Stone character sheet).
...
{IP_STYLE_ADAPT} — paste from active profile character.md (scene accents must not tint the character body).
...
生成前：读取 `handdrawn-style-dna.md`、`handdrawn-composition-patterns.md`、`handdrawn-qa-checklist.md`，选择一个结构类型；传小石头设定图（单锚点）；若老杨出镜，传双参考 `author-persona-panorama-handdrawn.png` + `author-persona-panorama.png`；写入 `{IP_DESC}` / `{IP_STYLE_ADAPT}`。
...
{IP_STYLE_ADAPT} — paste from active profile character.md.
...
Read author-persona-panorama-handdrawn.png + author-persona-panorama.png (+ author-persona-handdrawn-body.png for full-body; + author-persona-face-lock.png for multi-scene). Lao Yang = flat black-line hand-drawn mature Chinese man about 38 years old, Northern/Shandong face (NOT Korean idol), front or 3/4 view facing reader, pointing/annotating near title or conclusion. Six anchors: large light-gray thin-frame glasses / 3-5cm short straight dark hair / beige short-sleeve T + olive khaki shorts / thick lower lip / wheat skin / calm mature face. Body proportions match handdrawn-body when full body visible. FORBIDDEN: back view, rear silhouette, speech bubble only without face, gray curly white hair, old woman/elderly lady silhouette, generic businessman, thick black glasses, black long sleeves, oversized head/short legs. Little Stone still flat 2D #f39800 + primary-character-reference.png.
